...
@Component({
  selector: 'app-ivmeIki',
  templateUrl: './ivmeIki.component.html',
  styleUrls: ['./ivmeIki.component.css']
})
export class IvmeIkiComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.getIvme().subscribe((data)=>{
      ...
      data.map(x=>{
        ...
        var time =new Date(this.sensorValue1[2].waveform.timestamp).getTime(); 
        console.log(time)
        this.sensorValue1[2].waveform.wf_values.map((item, i) => {
